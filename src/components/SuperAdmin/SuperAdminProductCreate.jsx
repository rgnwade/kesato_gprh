...
class SuperAdminProductCreate extends Component {
  state = {
    data: [],
    notification: []
  };
  ...
  doSubmit = async () => {
    try {
      ...
      const result = await http.post(config.apiEndPoint + "product/create", {
        data: cleanData
      });
      if (result) {
          ...
          <div className="box-listing">
            <div className="text-with-search d-flex justify-content-between align-items-center">
              <h3 className="text-title mb-0 text-uppercase">
                <span>Ajout D'Un Nouveau produit</span>
...
export default SuperAdminProductCreate;
